...
static usb_sts_type class_out_handler(void *udev, uint8_t ept_num)
{
  ...
  for(index = 0; index < pcdc->g_rxlen; index++)
  {
    usb_rx_buffer[usb_rx_index] = pcdc->g_rx_buff[index];
    usb_rx_index = (usb_rx_index+1) & (usb_rx_buffer_size-1);
  }
...
/**
  ...
  * @retval error status
  */
error_status usb_vcp_send_data(void *udev, const uint8_t *send_data, uint16_t len)
{
